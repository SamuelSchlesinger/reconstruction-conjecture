...
| Laplacian characteristic polynomial | yes | Kelmans identity + constant-term recovery | Tutte 1979 [tutte79][tutte79]; Kelmans 1967 [kelmans67][kelmans67] |
...
| Tutte polynomial $T(G;x,y)$ | yes | rank-polynomial via Kelly counts on non-spanning subgraphs + spanning-term reduction | Tutte 1979 [tutte79][tutte79]; Bondy 1991 §5 [bondy91][bondy91]; Brylawski–Oxley 1992 [brylawskioxley92][brylawskioxley92] |
| Genus | unknown | — | open |
...
* Manvel [manvel74][manvel74] showed that trees on $n$ vertices are
  $k$-reconstructible for sufficiently small $k$ (verify exact threshold
  against Manvel 1974).
...
* **Chromatic number** $\chi(G)$. The chromatic polynomial is reconstructible
  (Tutte), and since $P(G, k) \in \mathbb{Z}[k]$ is reconstructible and
  $\chi(G) = \min\{ k \in \mathbb{Z}_{\ge 1} : P(G, k) > 0 \}$,
  **$\chi(G)$ is reconstructible**. [tutte79][tutte79] The common
  misconception that $\chi$ is open is flagged here for the reader.
...
* [`data/kelly_small.py`](data/kelly_small.py) — brute-force verification of
  Kelly's Lemma (induced form) on all labelled graphs on 5 vertices for
  $F = K_2$ and $F = P_3$.
* [`data/charpoly_deck.py`](data/charpoly_deck.py) — brute-force
  verification that $\phi'(G,x) = \sum_v \phi(G-v,x)$ on all labelled
  graphs up to 6 vertices.
...
### Script-to-theorem map

Each validation script is a finite-range sanity check on a specific
Lean/mathlib theorem or identity. The map:

| Script | Identity checked | Lean target |
|--------|------------------|-------------|
| `data/kelly_small.py` | $(n-k)\,s(F,G) = \sum_v s(F, G-v)$ (induced form of Kelly's Lemma) for $F \in \{K_2, P_3\}$ and $n = 5$ | `Reconstruction.KellyLemma` (induced-subgraph double-counting identity) |
| `data/kelly_small.py` ($F = K_2$ specialization) | $(n-2)\,m(G) = \sum_v m(G-v)$ | `Reconstruction.KellyEdgeCount` |
| `data/charpoly_deck.py` | $\phi'(G, x) = \sum_v \phi(G-v, x)$ in $\mathbb{Z}[x]$ | `Reconstruction.CharPolyFull` (derivative identity used by Tutte's charpoly reconstruction) |

These scripts do not prove anything in Lean; they rule out typos and
off-by-one errors in the statements by verifying the identities on
every labelled graph up to a fixed size.
