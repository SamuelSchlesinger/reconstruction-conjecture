...
vertices. This script tests the **induced** subgraph-count form of
Kelly's Lemma (counts of S with G[S] isomorphic to F, not counts of
...
number of k-element subsets S of V(H) with the induced subgraph H[S]
isomorphic to F.

Specializing to F = K_2: s(F, G) is just the edge count m(G) of G
(since the induced subgraph on two vertices is either an edge or not).

We also check, as an extra sanity check, the same identity for
F = P_3 (path on 3 vertices, k = 3), again using induced subgraph
counts, against all graphs on 5 vertices.
