...
## Sachs's coefficient theorem

The coefficients of $\phi(G, x)$ admit a combinatorial description in
terms of small subgraphs of $G$. This is essential for showing that
most coefficients of $\phi$ are already reconstructible as explicit
subgraph counts, without needing the derivative identity.

> **Theorem (Sachs 1964 [sachs64][sachs64]).** Write
> $\phi(G, x) = \sum_{k = 0}^{n} c_k\, x^{n - k}$. Then for each
> $0 \le k \le n$,
> $$c_k \;=\; \sum_{H \in \mathcal{E}_k(G)} (-1)^{p(H)}\, 2^{c(H)},$$
> where $\mathcal{E}_k(G)$ is the set of **elementary** subgraphs of $G$
> on exactly $k$ vertices — i.e. subgraphs each of whose connected
> components is either a single edge ($K_2$) or a cycle $C_\ell$ for
> some $\ell \ge 3$ — and for such $H$, $p(H)$ is the total number of
> components of $H$ and $c(H)$ is the number of components that are
> cycles.

This is sometimes colloquially called "Harary's coefficient formula" in
older surveys, but the correct attribution is to Sachs. (Harary's
1962 paper on closed walks gave a related but different identity
relating $\operatorname{tr}(A^k)$ to closed-walk counts.)

**Consequences for reconstruction.** Each elementary subgraph $H$ with
$k < n$ vertices is a (disjoint) union of edges and cycles on $< n$
vertices, so the sum defining $c_k$ is a $\mathbb{Z}$-linear combination
of counts $N(H, G)$ for non-spanning $H$. By Kelly's Lemma, each such
count is reconstructible, so $c_k$ is reconstructible for every
$k < n$. The coefficient $c_n$ (the constant term) is the only one
where some elementary subgraphs (e.g., a spanning Hamilton cycle $C_n$)
may be spanning, and this is precisely the case requiring a separate
...
characteristic polynomial $\mu(G, x) = \det(xI - L(G))$. A derivative
identity analogous to Tutte's for the adjacency characteristic
polynomial was given by Kelmans [kelmans67][kelmans67]; deleting a
vertex from $G$ changes the diagonal of $L(G)$ (the degrees of the
remaining vertices drop by one for each edge to the deleted vertex), so
the identity must account for this.

> **Kelmans identity (1967).** For every simple graph $G$ on $n$
> vertices,
> $$\frac{\partial}{\partial x}\mu(G, x) \;=\; \sum_{v \in V(G)}\,
>    \bigl[\mu(G - v,\; x) \;+\; \deg_G(v)\cdot q_v(G, x)\bigr],$$
> where $q_v(G, x)$ is a correction term expressible in terms of
> principal minors of $xI - L$ with row/column $v$ removed. (Exact form
> verify against Kelmans 1967.)

The take-away is that the right-hand side is a deck-determined
expression: each summand depends only on the card $G - v$ and the
degree $\deg_G(v)$, and $\deg_G(v)$ is reconstructible from the card
(via the edge-count identity). Integrating, together with a separate
constant-term recovery analogous to the adjacency case, recovers
$\mu(G, x)$.
Tutte [tutte79][tutte79] used this route to conclude that the Laplacian
characteristic polynomial is reconstructible, from which the number of
spanning trees follows immediately via Matrix-Tree.
...
[kelmans67]: ../sources.md#kelmans67
[schwenk74]: ../sources.md#schwenk74
[sachs64]: ../sources.md#sachs64
