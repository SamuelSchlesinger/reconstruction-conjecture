...
- `trace_adjMatrix_card_eq_of_support_counts_eq`
  ([`TopTrace.lean`](../../reconstruction-conjecture/Reconstruction/TopTrace.lean)):
  the missing top trace is reduced to equality of the proper-support and
  full-support top-length closed-walk counts.
...
proper induced subgraphs and should be reconstructible by Kelly after
regrouping by induced-subgraph isomorphism type. For `|S| = n`, a closed
walk of length `n` visiting all vertices is the Hamilton-cycle sector;
the intended next attack is a Kocay cover count for Hamilton cycles.
This route is more elementary than Sachs, but the Hamilton-cycle count is
the hard final subgoal.

**Lean plan (current bet: support split + Kocay).**

1. Use `TopTrace.lean` to reduce `tr(A^n)` to two counts:
   proper-support closed walks and full-support closed walks.
2. Regroup the proper-support exact-support sum by induced-subgraph
   isomorphism class, then apply Kelly's Lemma to show it agrees under
   `SameDeck`.
3. Identify the full-support top-length closed walks with rooted
   Hamiltonian cycles and recover their count using a Kocay cover identity.
4. Feed the resulting top-trace equality into
   `SameDeck.charPoly_coeff_zero_eq_of_trace_card_eq`.

**Fallback Lean plan (Sachs).**
...
**Current scaffold.** `SameDeck.charPoly_coeff_zero_eq_of_trace_card_eq`
is proved: once the top trace equality `tr(A^|V|)` is supplied, the
constant term follows by Cayley-Hamilton, the already-proved non-constant
coefficient equality, and trace reconstruction below `|V|`. The newer
`TopTrace.lean` scaffold proves the trace/closed-walk cardinality identity,
splits closed walks into proper-support and full-support pieces, expands
the proper-support piece as a sum over exact supports, and adds
`SameDeck.charPoly_coeff_zero_eq_of_support_counts_eq`.

---

## #B — Close `SameDeck.numComponents_eq` — **CLOSED 2026-04-17**

**Status.** Closed on 2026-04-17. See
[`ConnectedComponents.lean`](../../reconstruction-conjecture/Reconstruction/ConnectedComponents.lean).
The proof landed as a ~420-line three-way case split as originally
sketched. Retained below for historical reference.

**Payoff.** Unlocked the tree-case result and the disconnected-graph
reconstruction theorem. Also the obvious first "graph-theory-side"
reconstructibility lemma left unclosed.

**File.** `Reconstruction/ConnectedComponents.lean`.

**Difficulty.** Medium; ~200–400 lines (actual: ~420).
...
## #C — Close `SameDeck.iso_of_not_connected` (Kelly 1942) — **CLOSED**

**Status.** Closed in
[`Disconnected.lean`](../../reconstruction-conjecture/Reconstruction/Disconnected.lean)
using the descending component-count induction
`SameDeck.componentCount_eq_components_of_not_connected` from
[`Disconnected/ComponentCount.lean`](../../reconstruction-conjecture/Reconstruction/Disconnected/ComponentCount.lean),
then the assembly theorem `isoOfComponentCountEq`. Retained below for
...
**Difficulty.** Medium–high; actual closure came from the existing
component-count scaffold plus a descending induction over component size.
...
3. Assemble `G ≃g H` from a component-wise matching using the now-proved
   `isoOfComponentIsoEquiv`: choose a bijection between component sets
   preserving isomorphism type, then supply the component-level `≃g` maps.

**Risk.** Resolved. The delicate Step 2 is now packaged as
`SameDeck.componentCount_eq_components_of_not_connected`; Step 3 is
`isoOfComponentCountEq`.

**Verification.** `SameDeck.iso_of_not_connected` builds.
...
**Budget.** Roughly ~300 lines of Möbius-function-on-subgraph-poset
API (`edgeSubgraphPoset`, `Möbius`, inclusion–exclusion over
edge-subsets) before Lovász's bound is in reach; ~500–800 lines for
the Lovász threshold itself; +200 lines for the Nash-Williams
extension. Key references to cite in doc-comments:
[lovasz72][lovasz72] (the original ~1-page proof of the threshold)
and [nashwilliams78][nashwilliams78] (Nash-Williams' Lemma on
edge-reconstruction and its sharpening of the density bound).
...
| Rank | Target | Status (2026-04-17) | Rationale |
|------|--------|---------------------|-----------|
| ~~1~~ | ~~**#B** — `numComponents_eq`~~ | **CLOSED 2026-04-17** | Three-way case split landed. |
| ~~1~~ | ~~**#C** — `iso_of_not_connected`~~ | **CLOSED** | Kelly's disconnected-graph theorem landed. |
| 1 | **#A** — `charPoly_coeff_zero_eq` | Open | Upgrades the spectral half to "full charpoly". |
| 2 | **#E** — `decide`-certified small `n` | Open | Reproducible witness; engineering. |
| 3 | **#D** — edge-deck API + Lovász | Open | New direction, largest scope. |
