...
2. **Induced subgraph count.** `SimpleGraph.copyCount` is close but
   ...
   lemma.
3. **Newton's identities (matrix/trace form) / Faddeev–LeVerrier.**
   Confirmed against a pinned Mathlib tree on 2026-04-16: the only
   `Newton` file in Mathlib is
   `Mathlib.RingTheory.MvPolynomial.Symmetric.NewtonIdentities`,
   which proves Newton's identities *for symmetric polynomials in
   `MvPolynomial`*, not for matrix traces `tr(A^k)` / charpoly
   coefficients. A grep across
   `Mathlib/LinearAlgebra/Matrix/Charpoly/*.lean` and
   `Mathlib/LinearAlgebra/Matrix/**.lean` for `newton` and `faddeev`
   returns no matches. The sibling project's `Newton.lean` therefore
   supplies genuinely new content (~460 lines) that could be
   upstreamed with minor renaming to
...
5. **Multiset deck / deck type.** There is no `SimpleGraph.deck`
   ...
   requires quotienting subgraphs by isomorphism, which in turn wants
   an `IsomorphismClass` type. Confirmed 2026-04-16: grep of pinned
   Mathlib for `IsomorphismClass` yields only two files
   (`CategoryTheory.Skeletal` and
   `CategoryTheory.Limits.Shapes.ZeroMorphisms`), both in the
   category-theory skeleton machinery and neither supplying a
   `SimpleGraph`-specialised `IsomorphismClass`. A direct quotient
...
`v4.28.0` tree. The 2026-04-16 verification used the Mathlib tree
shared by the sibling Fortune project at
`number-theory/fortune/.lake/packages/mathlib/` (same pinned
`v4.28.0`).
