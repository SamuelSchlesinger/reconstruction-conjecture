...
over eighty years the conjecture remains open in full generality, but an
...
every graph and for every graph with more than `n · log_2 n` edges.
Computer verification now extends the conjecture through all graphs on at
most 13 vertices (McKay 2022) [mckay22][mckay22]. In contrast, the **digraph
reconstruction conjecture** is *false*: Stockmeyer (1977) exhibited infinite
families of tournament counterexamples, though these say nothing about the
undirected problem. This document is the umbrella for the current status and links out
...
| Regular graphs | 1977 | Bondy–Hemminger [bondyhemminger77][bondyhemminger77] | Immediate from Kelly's Lemma + degree-sequence identification; see [../invariants/index.md](../invariants/index.md) |
...
| Graphs with `> n · log_2 n` edges (edge-reconstruction) | 1977 | Müller [muller77][muller77] | See [edge-reconstruction](#edge-reconstruction) |
...
- **Regular graphs (Bondy–Hemminger 1977).** The degree sequence is
  reconstructible, so we know regularity and the common degree. Kelly's
  Lemma reconstructs the number of subgraphs isomorphic to each small
  graph, which pins down the full structure via inductive amalgamation
  [bondyhemminger77][bondyhemminger77].
...
- **Müller (1977).** Every graph with more than `n · log_2 n` edges is
...
- **Chordal graphs beyond interval** — not fully settled. (Interval graphs
  themselves are now known reconstructible, Heinrich et al. 2025
  [heinrich25][heinrich25].)
...
non-isomorphic tournaments with the same deck. The construction yields
counterexample tournaments at every order `n = 2^s + 2^t` with
`0 ≤ s < t`, giving smallest tournament orders 3, 5, 6, 9, 10, 12, 17,
18, ... (non-tournament digraph counterexamples also occur at small `n`).
See [stockmeyer81][stockmeyer81] for the expanded account (six related
families), and the 2009 erratum in *J. Graph Theory* **62**(2), 199–200.
...
- [computational-verification.md](computational-verification.md) —
  McKay's verification up through `n ≤ 13` (McKay 2022) and the tools used.
