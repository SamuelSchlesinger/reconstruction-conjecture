...
See also [`../invariants/spectral-invariants.md`](../invariants/spectral-invariants.md)
for a parallel discussion of the adjacency/Laplacian spectrum as
graph invariants and their deck-reconstructibility status.
...
subgraphs together with the **Sachs coefficient theorem**
[sachs64][sachs64]
...
Schwenk 1973 [schwenk73][schwenk73] proved the much sharper asymptotic
statement:

> **Schwenk 1973.** The fraction of unlabeled trees on $n$ vertices
> having a cospectral mate (i.e. a non-isomorphic tree with the same
> characteristic polynomial) tends to $1$ as $n \to \infty$.

In particular, cospectral non-isomorphic trees exist in abundance.
Hence:
...
- Can one reconstruct the **adjacency matrix up to permutation
...
- **Sachs coefficient theorem** [sachs64][sachs64] **for $\phi_G(x)$.**
  Requires `Matrix.det`, `Matrix.charpoly`, and a bijection between
  permutation expansions and elementary subgraph configurations. Mathlib
  has the determinant expansion; the combinatorial bijection is the new
  content.
