...
- **Random regular graphs.** Almost every $d$-regular graph (for $d$
  in an appropriate range) is widely expected to be reconstructible by
  the same 3-card mechanism as in the dense Erdős–Rényi model, via
  concentration of neighborhood-distinguishing statistics. We are not
  aware of a single canonical citation that states exactly this
  theorem in the literature; the general spectral / quasirandomness
  toolkit for random regular graphs (e.g. Kim, Sudakov, and Vu's work
  on spectra and neighborhood structure) supplies the ingredients but
  not, to our knowledge, the reconstruction statement itself. (Verify.)
