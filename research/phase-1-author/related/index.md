...
| 3 | **Set Reconstruction Conjecture (SRC)** | Every graph on `n ≥ 4` vertices is determined by the *set* (not multiset) of its `(n−1)`-cards. | **Open**; strictly stronger than RC; verified for `n ≤ 13` ([mckay97][mckay97]). | [hararyplantholt85][hararyplantholt85] |
...
```
   ...
   Digraph Recon (FALSE) ─────────╮
           │                       │
           ↓                       │ (motivated salvage)
   Tournament Recon (FALSE)        ↓
                                New Digraph Recon (open)
                               
                       ┌─────────────────────┐
                       │                     │
                       ▼                     │
   Set Reconstruction  ══ (Strictly stronger)══>  Reconstruction Conjecture (RC)
           (open)                                           │
                                                            │  Greenwell 1971
                                                            ▼
                                                    Edge Reconstruction (ERC)
                                                            │
                                                            │  ?  (open)
                                                            ▼
                                                   back to RC on connected, ≥ 4 edges
...
- **RC ⇒ ERC.** Greenwell showed that for graphs without isolated edges
  `(K₂)`, vertex-reconstructibility implies edge-reconstructibility
  [greenwell71][greenwell71].
...
1. **Distance-deck of a vertex-deleted family.** Record for each `v`
   the multiset `{d_{G−v}(x,y)}`. Used in the RC reduction that shows
   graphs of diameter `≥ 3` are reconstructible in many standard
   arguments, since distance profiles are recoverable from the ordinary
   deck (Kelly's Lemma).
