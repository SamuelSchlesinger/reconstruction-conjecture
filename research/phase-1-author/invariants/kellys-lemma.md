...
provided $|V(F)| = k < n$. Indeed $N(F, G) = \sum_{F'} s(F', G)$ where
$F'$ ranges over supergraphs of $F$ on the same vertex set (inclusion in
the lattice of graphs on $[k]$); apply the induced version term by term.

An equivalent inversion uses Möbius on the subgraph poset: if
$|V(F)| < n$, then $N(F, G)$ is reconstructible, and by Möbius inversion
$s(F, G)$ is a linear combination of subgraph counts $N(F', G)$ over $F'
\supseteq F$ on the same vertex set. Hence the two forms are equivalent.
