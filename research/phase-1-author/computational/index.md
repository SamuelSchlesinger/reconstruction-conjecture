...
2. From the degree sequence, determine the number of edges
   ...
   counting form).
3. Attempt to "fuse" two cards `G - u` and `G - v` along a common
   `(n-2)`-vertex subgraph that equals `G - {u, v}` in both. If the
   deck is legitimate the fusion succeeds and yields `G` up to
   isomorphism.
...
[krahem94][krahem94][hhrt09][hhrt09].
...
|-----------|-------------------------------------------|-----------------------------------------------|-----------|
| 3 | — (tournament ex. — verify) | 1 | Harary–Palmer (early) |
| 4 | — (tournament ex. — verify) | 1 | Harary–Palmer (early) |
| 5 | 1 tournament pair | 1 | Beineke–Parker (verify) |
| 6 | 3 tournament pairs | (several) | Beineke–Parker (verify) |
...
The small-order enumeration (`n ∈ {3, 4, 5, 6}`) predates Stockmeyer's
infinite families and is due to Harary–Palmer and Beineke–Parker;
exact counts should be cross-checked against Stockmeyer's 1977 paper
([stockmeyer77][stockmeyer77]) and its erratum and Kocay 1985
[kocay85][kocay85] before citing in any formalization work. Tan's
Oxford thesis [tan24][tan24] (chapter 2) gives the most recent
consolidated count; we mark the order-3/4/5/6 row "verify" in the
table because the exact number of pairs at each of those orders
varies by source convention (tournaments vs. loopless digraphs,
symmetric pairs, etc.).
