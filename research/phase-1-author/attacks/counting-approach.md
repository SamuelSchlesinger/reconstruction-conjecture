...
- Two regular non-isomorphic graphs with the same parameters (e.g. the two
  ...
  $4\times4$ rook graph) have the same counts of every proper subgraph
  up to small sizes but differ at larger subgraphs. For the reconstruction
  problem this is not a counterexample — the deck itself contains more
  data than just subgraph counts — but it bounds *pure counting*.
