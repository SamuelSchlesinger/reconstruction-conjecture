...
| Graphs with $\ge \binom{n-1}{2}+2$ edges | Müller edge-reconstruction | P (Müller 1977) | — |
...
| 3-regular (cubic) graphs | Structural / Kelly | O | High payoff |
...
| Edge-reconstruction of graphs with $\le \binom{n-1}{2}+1$ edges | Closes the remaining gap to Müller 1977 [muller77][muller77] | Nash-Williams 1978 [nashwilliams78][nashwilliams78] |
...
- **Algorithmic reconstruction.** Given the deck, recovering a model (when
  one exists) is polynomial-time *reducible* to graph isomorphism; with
  Babai's quasipolynomial GI algorithm [babai16][babai16], "can we
  distinguish two candidate reconstructions?" is tractable, though this
  gives no new information-theoretic leverage.
...
- **Category-theoretic reformulations.** Viewing the deck as a functor
  into $\mathbf{FinGraph}/{\cong}$ and asking about faithfulness reframes
  the problem but has not yet produced a new lever.
...
4. Closing the Müller edge-reconstruction gap for edge counts in
   $[n \log n / 2, \binom{n-1}{2}+1]$.
...
- **Tutte's charpoly reconstruction** (Tutte 1979 [tutte79][tutte79]):
  ...
  "headline" target.
- **Müller's $n \log n / 2$ edge-count theorem**: entropy / counting
  inequality; the probabilistic step reduces to explicit finite sums and
  Stirling-type bounds, which Mathlib supports.
- **Edge-reconstruction for graphs with many edges** via
  Lovász's formula (every graph with more than $\binom{n}{2}/2$ edges is
  edge-reconstructible): elementary Möbius / inclusion–exclusion; very
  clean Lean target.
