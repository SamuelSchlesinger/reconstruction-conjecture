...
## Regular graphs (folklore, attributed to Kelly)

**Theorem.** If `G` is regular, `G` is reconstructible.
...
reconstructive matching argument recovers `G`.

**Caveat.** Some accounts of this "folklore" result actually require
*strong* regularity or an extra hypothesis; Bondy's survey
[bondy91][bondy91] gives the precise statement. In the Lean
formalization this should be written out with care — the exact primary
source for "every regular graph is reconstructible, no extra
hypotheses" is worth double-checking against MathSciNet. (If the
genuine unconditional regular case is *not* known, then the open
status of e.g. 3-regular (cubic) graphs follows.)
...
| Regular graphs (standard statement) | Yes (folklore; verify primary source) | — |
| 3-regular (cubic) in full generality | Open | — |
