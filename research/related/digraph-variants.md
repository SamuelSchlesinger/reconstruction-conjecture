...
The **smallest case `s = 0, t = 1`** already gives a non-reconstructible
pair at **order `n = 2^0 + 2^1 = 3`**: the two distinct tournaments on
three vertices (the 3-cycle and the transitive tournament) have equal
vertex-decks, each card being the unique tournament on two vertices.
Further small cases come from `(s, t) = (0, 2), (1, 2), (0, 3), ...`
at orders `5, 6, 9, 10, 12, 17, 18, ...`. Concrete larger
counterexamples include certain *doubly regular* tournaments.
...
**Tournament Reconstruction Conjecture.** Every tournament on `n ≥ 3`
vertices is determined by its deck. — **False** by Stockmeyer's
theorem above, already at the minimal case `n = 3`.
...
1. **Stockmeyer's counterexamples**: the pair of tournaments on
   3 vertices (the 3-cycle and the transitive tournament) with the
   same deck but non-isomorphic. Excellent "refutation" target; small
   enough to `decide` once the graph API is in place.
