...
- **Superlinear lower bound** (Nýdl). [nydl92][nydl92] constructed
  infinite families of pairs of non-isomorphic graphs which are not
  reconstructible from their large-cardinality subgraphs, showing
  `M_ℓ` grows at least like `c · ℓ^{3/2}` and in fact superlinearly —
  definitely not `M_ℓ = 2ℓ + 1` in general.
...
- [spinoza-west19][spinoza-west19] gives the currently sharpest
  upper-bound framework for reconstruction from the `k`-deck of
  induced subgraphs; and — as an earlier Nýdl note —
  [nydl90][nydl90] treats the tree case (reconstruction of finite
  trees from small subtrees).
...
[spinoza-west19]: ../sources.md#spinoza-west19
